...
Both humans and tools use the a project's status (or proxies for its status,
such as recent activity) to determine whether the project is maintained or
otherwise suitable for inclusion as a software component.
...
1. Distributions can include *trove classifiers* in their metadata, as
   ...
   to indicate that the distribution's project is inactive.
   ...
   packaging ecosystems, older distributions can't have their classifiers
...
machine-readable, (2) general, (3) index-agnostic, and (4) updatable without
pushing a new distribution:

.. csv-table::
    :header: "Mechanism", "Machine-readable?", "General?", "Index-agnostic?", "Updatable?"
...
* When a project has one or more statuses, the index **SHALL** add each status
  to the ``project-status`` key at the top-level of the JSON response.
